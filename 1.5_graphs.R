...
#Figure 2 and 4
# read data
p_coef <- read_delim(".txt", "\t", escape_double = FALSE, trim_ws = TRUE)
p_coef$p500_c[p_coef$p500_p>0.05] <- 0
n_coef <- read_delim(".txt", "\t", escape_double = FALSE, trim_ws = TRUE)
n_coef$n500_c[n_coef$n500_p>0.05] <- 0
np_coef <- read_delim(".txt", "\t", escape_double = FALSE, trim_ws = TRUE)
np_coef$np500_c[np_coef$np500_p>0.05] <- 0

pop_1995 <- read_csv(".csv") %>% filter(year==1995)
pop_2016 <- read_csv(".csv") %>% filter(year==2016)

ndvi_1995 <- read_delim(".txt", 
                        "\t", escape_double = FALSE, trim_ws = TRUE) %>% filter(year == 1995)
ndvi_2000 <- read_delim(".txt", 
                        "\t", escape_double = FALSE, trim_ws = TRUE) %>% filter(year == 2000)
ndvi_2005 <- read_delim(".txt", 
                        "\t", escape_double = FALSE, trim_ws = TRUE) %>% filter(year == 2005)
ndvi_2010 <- read_delim(".txt", 
                        "\t", escape_double = FALSE, trim_ws = TRUE) %>% filter(year == 2010)
ndvi_2016 <- read_delim(".txt", 
...
AB <- ggarrange(A, B, ncol = 2, common.legend = T, legend = "right")

# 6C and 6D
pop <- read_csv(".csv") %>% .[,c(1:3,5)] %>%
  filter(year > 1994) %>% filter(year < 2017)
nat <- read_delim("geokoor_ndvi.txt", 
...
for(i in seq(1995, 2016)){
  ...
  ndvi_popdens<- rbind(ndvi_popdens,
                       c(i,
                         mean(na.omit(temp_val$n500[temp_val$count_500m<400])),
                         mean(na.omit(temp_val$n500[temp_val$count_500m>400 & temp_val$count_500m<1200])),
                         mean(na.omit(temp_val$n500[temp_val$count_500m>1200]))))
}

ndvi_popdens <- as.data.frame(ndvi_popdens)
colnames(ndvi_popdens) <- c("year", "Below 400", "From 400 to 1200", "Over 1200")
