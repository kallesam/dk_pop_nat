...
denmark <- readOGR(dsn = "adm", layer = "DK_adm1_32N")
water <- readOGR(dsn = "water", layer = "dk_water_32N")

# directory of source images
# redo with both May-Aug and July
dk_fold <- "RMA_dk_july" # denmark
bh_fold <- "RMA_bornholm_july" # bornholm
# create list of files
dk_list <- list.files(dk_fold, full.names = T)
bh_list <- list.files(bh_fold, full.names = T)
...
outputnames_255_m <- paste("ndvi_255_", seq(2017, 2018), "_m.tif", sep = "")
outputnames_495_m <- paste("ndvi_495_", seq(1995, 2016), "_m.tif", sep = "")
outputnames_1005_m <- paste("ndvi_1005_", seq(2017, 2018), "_m.tif", sep = "")
