...
pop_9516$nbbd <- "From 400 to 1200"
...
pop_9516 <- pop_9516 %>% gather("year", "pop", -x, -y, -density)
pop_9516 <- pop_9516 %>%
...
colnames(nat_9516)[3:4] <- c("1995", "2016")
...
A <- natpop_9516 %>%
  ...
  xlim(1.5, 4) +
  ylim(0.25, 0.65) +
  #scale_fill_paletteer_c(rev("gameofthrones::stark"), name ="Density") +
...
C <- ndvi_popdens_s %>%
  ...
  geom_line() +
  scale_color_manual(values = wes_palette("Darjeeling1", n = 3)) +
  labs(title = "C", x = "Year", y  = "NDVI relative to 1995", color = "Population") +
  theme_light()

BC <- ggarrange(B, C, ncol = 2, common.legend = T, legend = "right")
BC
