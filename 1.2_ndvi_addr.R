...
# NDVI data
ndvi.fold <- "ndvi_july" # do once for all summer, once for july
...
for(i in seq_along(file_list)){
  ...
  print("data merged")
  write.table(addr@data, file = "geokoor_ndvi_july.txt", sep = "\t", row.names = F)
}

# save in long format instead
n250 <- read_delim("geokoor_ndvi.txt", "\t",
...
n500 <- read_delim("geokoor_ndvi_july.txt", "\t",
                   escape_double = FALSE, trim_ws = TRUE) %>%
...
n1000 <- read_delim("geokoor_ndvi.txt", "\t",
                    escape_double = FALSE, trim_ws = TRUE) %>%
...
write.table(ndvi, file = "ndvi_long.txt", sep = "\t", row.names = F)
