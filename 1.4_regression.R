...
var_calc <- function(var) {
  tmp_data <- data[,c(3:7,var)] # make sure columns are right
...
nat_s <- read_delim("", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  filter(year>1994) %>%
  SpatialPointsDataFrame(coords =  .[,c(1:2)], proj4string = CRS(crs))

# create list of municipalities from folder
mun.fold <- ""
file_list = list.files(mun.fold)
file_names <- file_list %>% str_replace(".gpkg", "")
setwd(mun.fold)
